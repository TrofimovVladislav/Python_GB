...
# 2. HARD! Найдите корни квадратного уравнения Ax² + Bx + C = 0 ) 
# с помощью математических формул нахождения корней квадратного уравнения.
# HARD - уравнение вводится пользователем с клавиатуры.
...
def check_equation():
    ...
    pattern = '([-0-9]\d+x2[-+0-9]\d+x[-+0-9]\d+=0)'  # шаблон для Ax2 + Bx + C = 0

    # pattern = '([-0-9]\d+x2=0)'                     # шаблон для Ax2 = 0
    # pattern = '([-0-9]\d+x2[-+0-9]\d+=0)'           # шаблон для Ax2 + C = 0
    # pattern = '([-0-9]\d+x2[-+0-9]\d+x=0)'          # шаблон для Ax2 + Bx = 0

    # # !!! Решение для 2, 3, и 4 шаблонов в данном коде не реализованы !!! 
